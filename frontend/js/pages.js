                ...
                ${otdRes.data.map(m => `
                  <div style="flex-shrink:0;width:200px;border-radius:var(--radius-lg);overflow:hidden;background:var(--bg-tertiary);cursor:pointer" onclick="Router.navigate('#timeline')">
                    <div style="height:140px;overflow:hidden">
                      <img src="${assetUrl(m.mediaUrl)}" style="width:100%;height:100%;object-fit:cover" onerror="this.style.background='var(--bg-tertiary)'">
                    </div>
...
Pages.chat = {
  ...
  async render(root) {
    // ── FIXED: Show "no couple" message if not connected ──
    const couple = App.coupleInfo || await api.users.couple().then(r => r.data).catch(() => null);
    if (!couple) {
      root.innerHTML = `
        <div class="page-container">
          <div class="empty-state" style="padding:var(--space-16)">
            <div class="empty-icon">💬</div>
            <div class="empty-title">Chat Not Available</div>
            <div class="empty-desc">Connect with your partner first to start chatting</div>
            <button class="btn btn-primary" style="margin-top:var(--space-6)" onclick="showInviteModal()">💌 Connect with Partner</button>
          </div>
        </div>`;
      return;
    }
    ...
    root.innerHTML = `
      <div class="chat-container">
        <div class="chat-header glass-card no-hover" style="margin:0;border-radius:0;border-left:none;border-right:none;border-top:none;padding:var(--space-4) var(--space-6);display:flex;align-items:center;gap:var(--space-3)">
          ...
          <div>
            <div style="font-weight:700" id="partner-name">${escapeHtml(couple.partnerDisplayName)}</div>
    ...
    if (couple.partnerAvatarUrl) {
      document.getElementById('partner-avatar').innerHTML = `<img src="${assetUrl(couple.partnerAvatarUrl)}" alt="">`;
    } else {
  ...
  async loadMessages() {
    try {
      ...
      if (container) container.innerHTML = `<div class="empty-state"><div class="empty-icon">💬</div><div class="empty-title">Start your conversation</div></div>`;
    }
  },
        ...
        <div>
          <div class="message-bubble-list">
            <div class="message-bubble" ondblclick="Pages.chat.quickReact(${m.id})" title="Double-click to react">
              ${m.mediaUrl ? `<div style="margin-bottom:var(--space-2)"><img src="${assetUrl(m.mediaUrl)}" style="max-width:240px;border-radius:12px" onerror="this.style.display='none'"></div>` : ''}
  ...
  bindWsEvents() {
    window.addEventListener('ws:message', e => { this.addMessage(e.detail); });
    ...
    return `
      <div class="glass-card memory-card" onclick="Pages.timeline.openMemory(${m.id})">
        <div class="memory-card-image-wrap" style="position:relative">
          ${m.mediaType === 'VIDEO'
            ? `<video src="${assetUrl(m.mediaUrl)}" class="memory-card-image" muted></video>`
            : m.mediaType === 'VOICE'
            ? `<div class="memory-card-image" style="display:flex;align-items:center;justify-content:center;font-size:3rem">🎙️</div>`
            : `<img src="${assetUrl(m.mediaUrl)}" class="memory-card-image" alt="${escapeHtml(m.title)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=memory-card-image style=display:flex;align-items:center;justify-content:center;font-size:3rem>📸</div>'">`}
    ...
    try {
      ...
      modal(`📸 ${escapeHtml(m.title)}`, `
        <div style="margin-bottom:var(--space-4);border-radius:var(--radius-lg);overflow:hidden">
          ${m.mediaType === 'VIDEO'
            ? `<video src="${assetUrl(m.mediaUrl)}" controls style="width:100%;max-height:400px"></video>`
            : m.mediaType === 'VOICE'
            ? `<audio src="${assetUrl(m.mediaUrl)}" controls style="width:100%"></audio>`
            : `<img src="${assetUrl(m.mediaUrl)}" style="width:100%;max-height:400px;object-fit:contain" alt="">`}
        </div>
...
Pages.dates = {
  ...
  showAddModal() {
    modal('📅 Add Special Date',
      `<div style="display:flex;flex-direction:column;gap:var(--space-4)">
        <div class="form-group"><label class="form-label">Event Title *</label><input class="form-input" id="event-title" placeholder="Our Anniversary, Birthday..."></div>
        <div class="form-group"><label class="form-label">Description</label><input class="form-input" id="event-desc" placeholder="Optional details"></div>
        <div class="form-group"><label class="form-label">Date *</label><input type="date" class="form-input" id="event-date"></div>
        ...
        <div class="form-group"><label class="form-label">Color</label><input type="color" id="event-color" value="#FF6B9D" style="width:60px;height:40px;border:none;cursor:pointer;background:none;border-radius:8px"></div>
        ...
        <div class="form-group"><label class="form-label">Reminder (days before)</label><input type="number" class="form-input" id="event-reminder" value="7" min="1" max="90"></div>
      </div>`,
      `<button class="btn btn-ghost" onclick="this.closest('.modal-backdrop').remove()">Cancel</button>
...
Pages.notes = {
  ...
  async openNote(id) {
    try {
      ...
      modal(n.title || '💌 Love Note', `
        ...
        ${n.mediaUrl ? `<div style="margin-top:var(--space-4)"><img src="${assetUrl(n.mediaUrl)}" style="width:100%;border-radius:var(--radius-md)" onerror="this.remove()"></div>` : ''}`,
  ...
  showWriteModal() {
    modal('✍️ Write a Love Note', `
      <div style="display:flex;flex-direction:column;gap:var(--space-4)">
        <div class="form-group"><label class="form-label">Title (optional)</label><input class="form-input" id="note-title" placeholder="A little surprise..."></div>
        <div class="form-group"><label class="form-label">Your message *</label><textarea class="form-textarea" id="note-content" placeholder="Write from the heart..." style="min-height:140px"></textarea></div>
        ...
        <div style="background:var(--bg-overlay);border-radius:var(--radius-md);padding:var(--space-4)">
          ...
          <div id="note-deliver-wrap" style="display:none"><input type="datetime-local" class="form-input" id="note-deliver-at"></div>
        </div>
        <div style="background:var(--bg-overlay);border-radius:var(--radius-md);padding:var(--space-4)">
          ...
          <div id="note-unlock-wrap" style="display:none"><input type="datetime-local" class="form-input" id="note-unlock-at"></div>
        </div>
      </div>`,
      `<button class="btn btn-ghost" onclick="this.closest('.modal-backdrop').remove()">Cancel</button>
...
Pages.profile = {
          ...
          <div class="glass-card no-hover" style="padding:var(--space-8);text-align:center">
            <div style="position:relative;display:inline-block;margin-bottom:var(--space-5)">
              <div class="avatar avatar-xl" style="margin:auto" id="profile-avatar">
                ${user.avatarUrl ? `<img src="${assetUrl(user.avatarUrl)}" alt="">` : (user.displayName?.charAt(0) || '?')}
              </div>
              ...
              <div class="glass-card no-hover" style="padding:var(--space-6)">
                ...
                <div style="display:flex;align-items:center;gap:var(--space-4);margin-bottom:var(--space-5)">
                  <div class="avatar avatar-lg">
                    ${couple.partnerAvatarUrl ? `<img src="${assetUrl(couple.partnerAvatarUrl)}" alt="">` : (couple.partnerDisplayName?.charAt(0) || '?')}
                  </div>
                  <div>
            ...
            <div class="glass-card no-hover" style="padding:var(--space-6)">
              ...
              <div style="display:flex;flex-direction:column;gap:var(--space-3)">
                <div style="display:flex;justify-content:space-between;padding:var(--space-3) 0;border-bottom:1px solid var(--divider)">
                  <span class="text-muted">Email</span><span style="font-weight:500">${escapeHtml(user.email)}</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:var(--space-3) 0;border-bottom:1px solid var(--divider)">
                  <span class="text-muted">Username</span><span style="font-weight:500">@${escapeHtml(user.username)}</span>
                </div>
                <div style="display:flex;justify-content:space-between;padding:var(--space-3) 0">
                  <span class="text-muted">Member since</span><span style="font-weight:500">${formatDate(user.createdAt)}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  showEditModal() {
    ...
    modal('✏️ Edit Profile', `
      <div style="display:flex;flex-direction:column;gap:var(--space-4)">
        <div class="form-group"><label class="form-label">Display Name</label><input class="form-input" id="edit-name" value="${escapeHtml(user?.displayName || '')}"></div>
        <div class="form-group"><label class="form-label">Bio</label><textarea class="form-textarea" id="edit-bio">${escapeHtml(user?.bio || '')}</textarea></div>
      </div>`,
      `<button class="btn btn-ghost" onclick="this.closest('.modal-backdrop').remove()">Cancel</button>
  ...
  async uploadAvatar(input) {
    ...
    try {
      ...
      if (avatar) avatar.innerHTML = `<img src="${assetUrl(res.data.url)}" alt="">`;
...
async function joinWithCode() {
  ...
  try {
    await api.auth.joinCouple(code);
