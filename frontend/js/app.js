...
const WS = {
  ...
  connect(coupleId) {
    ...
    const socket = new SockJS('https://soulsync-2-dxl8.onrender.com/api/ws');
...
const App = {
  ...
  async init() {
    ...
    // Refresh user from API to get latest avatar
    try {
      const freshUser = await api.users.me();
      if (freshUser.data) {
        api.setUser(freshUser.data);
        this.user = freshUser.data;
        document.getElementById('sidebar').innerHTML = buildSidebar(freshUser.data);
      }
    } catch {}
  ...
  async toggleNotifPanel() {
    ...
    try {
      ...
      document.getElementById('notif-list').innerHTML = '<div style="padding:20px;color:var(--text-muted);text-align:center">Could not load notifications</div>';
    }
  },
